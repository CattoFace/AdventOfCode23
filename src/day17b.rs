...
fn solve_file(text: String) -> u32 {
    ...
    let mut visited = bitvec![0;text.len()*4*10]; // 4 directions, 10 move_lengths
    ...
    loop {
        ...
        if curr_district.y != 0
            ...
            && (!matches!(curr_district.direction, Up) || curr_district.move_length < 10)
            && (matches!(curr_district.direction, Up) || curr_district.move_length >= 4)
        {
            queue.push(next_district(text, &curr_district, Up, width));
        }
        if curr_district.y != height - 1
            && !matches!(curr_district.direction, Up)
            && (!matches!(curr_district.direction, Down) || curr_district.move_length < 10)
            && (matches!(curr_district.direction, Down) || curr_district.move_length >= 4)
        {
            queue.push(next_district(text, &curr_district, Down, width));
        }
        if curr_district.x != 0
            && !matches!(curr_district.direction, Right)
            && (!matches!(curr_district.direction, Left) || curr_district.move_length < 10)
            && (matches!(curr_district.direction, Left) || curr_district.move_length >= 4)
        {
            queue.push(next_district(text, &curr_district, Left, width));
        }
        if curr_district.x != width - 1
            && !matches!(curr_district.direction, Left)
            && (!matches!(curr_district.direction, Right) || curr_district.move_length < 10)
            && (matches!(curr_district.direction, Right) || curr_district.move_length >= 4)
        {
