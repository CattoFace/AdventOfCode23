...
pub fn solve_day() -> u16 {
    // solve_file(read_to_string("inputs/day21_test2.txt").unwrap(), 40)
...
fn solve_file(text: String, steps: u16) -> u16 {
    ...
    queue.push_back((start / width, start % width));
    ...
    for step in 0..steps {
        ...
        for _ in 0..iteration_size {
            let (x, y) = queue.pop_front().unwrap();
            {
                let new_x = x + 1;
                let new_y = y;
                let coord = new_x + new_y * width;
                if text[coord] == b'.' && !visited[coord] {
                    visited[coord] = true;
...
mod tests {
    ...
    fn solve_test() {
        assert_eq!(
            solve_file(read_to_string("inputs/day21_test.txt").unwrap(), 64),
            16
        );
        assert_eq!(
            solve_file(read_to_string("inputs/day21.txt").unwrap(), 64),
